...
The Utils module provides centralized configuration management, validation utilities, and common helper functions for the CV processing system. It manages the centralized `assistants.json` configuration file and provides a clean interface for accessing assistant prompts, schemas, and settings across all modules.

### **Core Responsibilities:**
- Centralized assistant configuration management via `assistants.json`
- Environment variable substitution and validation
- JSON schema loading and validation
- Assistant prompt management with variable interpolation
...
- **Load Assistant Configuration:** Parse and load `assistants.json` with environment variable substitution
- **Prompt Management:** Retrieve assistant prompts with dynamic variable interpolation
- **Schema Management:** Load and validate JSON schemas for assistant responses
- **Environment Handling:** Support different configurations for dev/staging/prod environments

### **2.2 Validation and Health Checks**
- **Configuration Validation:** Validate `assistants.json` structure and required fields
- **Environment Variable Validation:** Ensure all required environment variables are present
- **Schema Validation:** Validate assistant responses against defined schemas
- **Health Checks:** Verify system configuration health

### **2.3 Common Utilities**
- **Text Processing:** Common text manipulation and cleaning functions
- **Data Validation:** Input validation and sanitization utilities
- **Error Handling:** Standardized error response creation
- **Logging Helpers:** Utility functions for consistent logging
...
### **3.3 Assistant Configuration Access**
```python
def load_assistant_config(self, assistant_name: str) -> dict:
    """
    Load complete configuration for specific assistant
    """
...
def get_assistant_prompt(self, assistant_name: str, **kwargs) -> str:
    """
    Get formatted assistant prompt with variable substitution
    """
...
def get_assistant_settings(self, assistant_name: str) -> dict:
    """
    Get model configuration settings for assistant
    """
    assistant_config = self.load_assistant_config(assistant_name)
...
### **3.4 Validation and Health Checks**
```python
def validate_configuration(self) -> dict:
    """
...
## **4. Global Module Functions**

### **4.1 Configuration Management Functions**
```python
...
### **6.1 assistants.json Schema Validation**
```python
...
- **Manages:** `assistants.json` configuration file
- **Provides:** Centralized configuration, validation, and utility functions
- **Environment:** Integrates with Azure Functions environment variables

## **8. Usage Examples**

### **8.1 Loading Assistant Configuration**
```python
from shared_code.utils import load_assistant_config, get_assistant_prompt

# Load complete assistant configuration
config = load_assistant_config("cv_data_extractor")

# Get formatted prompt with variables
prompt = get_assistant_prompt("cv_data_extractor", 
...
### **8.2 Validation**
```python
from shared_code.utils import validate_system_configuration, ValidationUtils
...
# Validate email
if ValidationUtils.validate_email("user@example.com"):
    print("Valid email")
```
...
**Next Steps:**
- Implement ConfigurationManager class with all methods
- Create comprehensive validation logic
- Add utility functions for common operations
- Create assistants.json configuration file
- Integrate with other modules for centralized configuration access
